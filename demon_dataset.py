...
class DeMoNDataset(tud.Dataset):
        ...
        for scene in self.scenes:
            ...
            for left_idx in range(len(images)):
                ...
                for right_idx in shifts:
                    ...
                    T_right_in_left = np.dot(T_world_in_left, np.linalg.inv(T_world_in_right))
    ...
    def __getitem__(self, idx):
        ...
        left_image = np.array(Image.open(raw_sample["left_filename"]), dtype=np.float32)
        ...
        for idx in range(len(raw_sample["right_filename"])):
            ...
            right_images.append(np.array(Image.open(raw_sample["right_filename"][idx]), dtype=np.float32))
